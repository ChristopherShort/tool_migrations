...
def get_downloads_page_url(url, allow_redirects=True):
    ...
    url_details_page = [
        url for url in r.html.absolute_links if 'DetailsPage' in url]

    if len(url_details_page) > 1:
        print(url_details_page)
        raise ValueError(
            'Chris: More than 1 link found for Downloads/DetailsPage')
...
def is_file_type_downloadable(url):
    ...
    if content_type.lower() in ['application/vnd.ms-excel',
                                'application/x-zip'
                                ]:
        return True
    return False
...
def gen_ABS_3412(file_path, calendar=None):
    ...
    if not isinstance(calendar, bool):
        raise ValueError(
            'Chris: boolean for calendar or financial year not set')
    ...
    for sheet_name, df_ in sheets_dict.items():
        ...
        df_.columns = ['state', 'major_groupings',
                       'minor_groupings', 'nom_arrival', 'nom_departure', 'nom']
        ...
        df_.loc[:, 'minor_groupings'] = df_.loc[:,
                                                'minor_groupings'].fillna(value='Total')
        df_ = df_.dropna(axis='rows', thresh=4)
        df_.state = df_.state.str.replace(r'Australia\([^\)]\)', 'Australia', regex=True)
...
def ASGS_definitions(dict_folder=DICT_FOLDER):
    ...
    code_2_name_dict = {'SA2_MAINCODE_2016': 'SA2_NAME_2016',
...
def pop_component_definitions():
    component_2_name_dict = {'1': 'births',
                             '2': 'deaths',
                             '3': 'natural_increase',
                             '4': 'nim_arrivals',
                             '5': 'nim_departures',
