...
def read_abs_meta_data(folder_path=data_abs_path,
    ...
    if meta.iloc[:, 0].isna()[0]:
        ...
        meta.iloc[0, 0] = meta.iloc[0, 0].replace(' *> ', '', regex=True)  # wonder what ABS workbooks needed this?
...
def set_fin_year_axis(ax, rotation=0, ha='center', end_of_fin_year=True, every_year=True):
    '''
    Parameters:
    -----------
    ...
    if not every_year:
        ...
        labels_old = [tick.get_text() for tick in ax.xaxis.get_ticklabels()]
        labels_new = [t.get_text() if i % 2 else ""
